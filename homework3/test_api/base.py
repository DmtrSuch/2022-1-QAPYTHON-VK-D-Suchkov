...
class ApiBase:
    ...
    def setup(self, api_client):
        ...
        if self.authorize:
            self.api_client.post_login(self.api_client.user,self.api_client.password)
    ...
    def campaign(self, repo_root):
        ...
        file = os.path.join(repo_root, 'test_api', 'pict.jpg')
        image_id = self.upload_image(file)
        campaign_id = self.create_campaign(name=fake.lexify(text= '?' * 10), image_id=image_id, url_id=url_id)
        yield campaign_id
